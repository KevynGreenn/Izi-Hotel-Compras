...
// Função 1: Envia e-mail para o APROVADOR
async function enviarEmailAprovacao(emailDestino, token) {
    const frontendUrl = process.env.FRONTEND_URL;
    if (!frontendUrl) {
        console.error("ERRO CRÍTICO: FRONTEND_URL não definido. E-mail de aprovação não enviado.");
        return;
    }
...
// --- NOVIDADE AQUI ---
// Função 2: Envia e-mail de status para o REQUISITANTE
async function enviarEmailStatus(requisicao) {
    if (!requisicao.email_solicitante) {
        console.log(`Requisição ${requisicao.id} sem e-mail do solicitante. Notificação não enviada.`);
        return;
    }

    const statusText = requisicao.status === 'Aprovada' ? 'APROVADA' : 'REJEITADA';
    const subject = `Sua Requisição de Compra foi ${statusText}`;
    const body = `
...
app.post('/api/requisicao', async (req, res) => {
  try {
    // Adicionado o campo 'email' que virá do formulário
    ...
    const query = `
      ...
      RETURNING *`;
    // Adicionado 'email' ao array de valores
...
app.post('/api/requisicao/:token/aprovar', async (req, res) => {
  try {
    ...
    // --- NOVIDADE AQUI ---
    // Após aprovar, envia o e-mail de status para o solicitante
...
app.post('/api/requisicao/:token/rejeitar', async (req, res) => {
  try {
    ...
    // --- NOVIDADE AQUI ---
    // Após rejeitar, envia o e-mail de status para o solicitante
    await enviarEmailStatus(result.rows[0]);
